...
PlasmoidItem {
    ...
    function showNautilusWindows() {
        if (nautilusWindowIds.length > 0 && effectWatcher.registered) {
            backend.activateWindowView(nautilusWindowIds)
        } else if (nautilusWindowIds.length > 0) {
            // Fallback: activează prima fereastră dacă WindowView nu este disponibil
            var cmd = "xdotool windowactivate " + nautilusWindowIds[0]
    ...
    function showAllWindows() {
        // Activează și pune focus pe toate ferestrele Nautilus
        // Găsește toate ferestrele Nautilus și le activează secvențial, apoi lasă focus pe ultima
        var cmd = "xdotool search --class 'nautilus' | while read wid; do xdotool windowactivate \"$wid\" 2>/dev/null; sleep 0.05; done; xdotool search --class 'nautilus' | tail -1 | xargs -I {} xdotool windowactivate {} 2>/dev/null"
        executable.connectSource(cmd)
    }
